/* src/PNotifyDesktopComponent.html generated by Svelte v3.18.1 */
import { SvelteComponent, init as init_1, safe_not_equal } from "svelte/internal";

import PNotify from "./PNotifyCore";
const key = "Desktop";

const defaults = {
	// Display the notification as a desktop notification.
	desktop: false,
	// If desktop notifications are not supported or allowed, fall back to a regular notice.
	fallback: true,
	// The URL of the icon to display. If false, no icon will show. If null, a default icon will show.
	icon: null,
	// Using a tag lets you update an existing notice, or keep from duplicating notices between tabs.
	// If you leave tag null, one will be generated, facilitating the 'update' function.
	// see: http://www.w3.org/TR/notifications/#tags-example
	tag: null,
	// Optionally display a different title for the desktop.
	title: null,
	// Optionally display different text for the desktop.
	text: null,
	// Any additional options to be passed to the Notification constructor.
	options: {}
};

function requestPermission() {
	if (typeof Notification !== "undefined" && "requestPermission" in Notification) {
		Notification.requestPermission();
	} else if ("webkitNotifications" in window) {
		window.webkitNotifications.requestPermission();
	}
}

const Notification = window.Notification;

let notify = (title, options, onclick, onclose) => {
	// Memoize based on feature detection.
	if ("Notification" in window) {
		notify = (title, options, onclick, onclose) => {
			const notice = new Notification(title, options);

			if ("NotificationEvent" in window) {
				notice.addEventListener("notificationclick", onclick);
				notice.addEventListener("close", onclose);
			} else if ("addEventListener" in notice) {
				notice.addEventListener("click", onclick);
				notice.addEventListener("close", onclose);
			} else {
				notice.onclick = onclick;
				notice.onclose = onclose;
			}

			return notice;
		};
	} else if ("mozNotification" in navigator) {
		notify = (title, options, onclick, onclose) => {
			// Gecko < 22
			const notice = navigator.mozNotification.createNotification(title, options.body, options.icon).show();

			notice.onclick = onclick;
			notice.onclose = onclose;
			return notice;
		};
	} else if ("webkitNotifications" in window) {
		notify = (title, options, onclick, onclose) => {
			const notice = window.webkitNotifications.createNotification(options.icon, title, options.body);
			notice.onclick = onclick;
			notice.onclose = onclose;
			return notice;
		};
	} else {
		notify = (title, options, onclick, onclose) => {
			return null;
		};
	}

	return notify(title, options, onclick, onclose);
};

function checkPermission() {
	if (typeof Notification !== "undefined" && "permission" in Notification) {
		return Notification.permission === "granted";
	} else if ("webkitNotifications" in window) {
		return window.webkitNotifications.checkPermission() == 0; // eslint-disable-line eqeqeq
	}

	return false;
}

let _permission = checkPermission();

function instance($$self, $$props, $$invalidate) {
	let { _notice = null } = $$props;
	let { desktop = defaults.desktop } = $$props;
	let { fallback = defaults.fallback } = $$props;
	let { icon = defaults.icon } = $$props;
	let { tag = defaults.tag } = $$props;
	let { title = defaults.title } = $$props;
	let { text = defaults.text } = $$props;
	let { options = defaults.options } = $$props;
	let _desktop;
	let _oldAnimation;
	let _icon;
	let _tag;

	function init() {
		// Animation should always be 'none' for desktop notices, but remember
		// the old animation so it can be recovered.
		$$invalidate(12, _oldAnimation = _notice.animation);

		if (!desktop) {
			return;
		}

		_permission = checkPermission();

		if (!_permission) {
			// Keep the notice from opening if fallback is false.
			if (!fallback) {
				$$invalidate(0, _notice.autoOpen = false, _notice);
			}

			return;
		}

		_notice.addModuleClass("ui-pnotify-desktop-hide");
		$$invalidate(0, _notice.animation = "none", _notice);
		genNotice();
	}

	function beforeOpen() {
		if (desktop && !_permission) {
			requestPermission();
		}

		if (_disabled) {
			return;
		}

		if (_desktop && "show" in _desktop) {
			_notice.setModuleIsNoticeOpen(true);
			_desktop.show();
		}
	}

	function beforeClose() {
		if (_disabled) {
			return;
		}

		if (_desktop && "close" in _desktop) {
			_desktop.close();
			_notice.setModuleIsNoticeOpen(false);
		}
	}

	function genNotice() {
		if (icon === null) {
			switch (_notice.type) {
				case "error":
					_icon = "data:image/png;base64,";
					break;
				case "success":
					_icon = "data:image/png;base64,";
					break;
				case "info":
					_icon = "data:image/png;base64,";
					break;
				case "notice":
				default:
					_icon = "data:image/png;base64,";
					break;
			}
		} else if (icon === false) {
			_icon = null;
		} else {
			_icon = icon;
		}

		if (!_tag || tag !== null) {
			_tag = tag === null
			? "PNotify-" + Math.round(Math.random() * 1000000)
			: tag;
		}

		const desktopOptions = { body: text || _notice.text, tag: _tag };

		if (!_notice.hide) {
			desktopOptions.requireInteraction = true;
		}

		if (_icon !== null) {
			desktopOptions.icon = _icon;
		}

		Object.apply(desktopOptions, options);

		_desktop = notify(
			title || _notice.title,
			desktopOptions,
			() => {
				_notice.fire && _notice.fire("click", { target: _desktop });
			},
			() => {
				_notice.close && _notice.close();
			}
		);

		if (!("close" in _desktop) && "cancel" in _desktop) {
			_desktop.close = () => {
				_desktop.cancel();
			};
		}
	}

	$$self.$set = $$props => {
		if ("_notice" in $$props) $$invalidate(0, _notice = $$props._notice);
		if ("desktop" in $$props) $$invalidate(1, desktop = $$props.desktop);
		if ("fallback" in $$props) $$invalidate(2, fallback = $$props.fallback);
		if ("icon" in $$props) $$invalidate(3, icon = $$props.icon);
		if ("tag" in $$props) $$invalidate(4, tag = $$props.tag);
		if ("title" in $$props) $$invalidate(5, title = $$props.title);
		if ("text" in $$props) $$invalidate(6, text = $$props.text);
		if ("options" in $$props) $$invalidate(7, options = $$props.options);
	};

	let _disabled;

	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*fallback, desktop*/ 6) {
			$: $$invalidate(15, _disabled = !_permission && fallback || !desktop);
		}

		if ($$self.$$.dirty & /*_notice, _disabled, _oldAnimation*/ 36865) {
			$: {
				if (_notice.animation !== "none") {
					$$invalidate(12, _oldAnimation = _notice.animation);
				}

				// This is necessary so desktop notices don't cause spacing problems
				// when positioning.
				if (_notice.getAnimatingClass() !== "" && !_disabled) {
					_notice.setAnimatingClass("");
				}

				if (_disabled && _notice.hasModuleClass("ui-pnotify-desktop-hide")) {
					_notice.removeModuleClass("ui-pnotify-desktop-hide");
					$$invalidate(0, _notice.animation = _oldAnimation, _notice);
				} else if (!_disabled && !_notice.hasModuleClass("ui-pnotify-desktop-hide")) {
					_notice.addModuleClass("ui-pnotify-desktop-hide");
					$$invalidate(0, _notice.animation = "none", _notice);
					genNotice();
				}
			}
		}
	};

	return [
		_notice,
		desktop,
		fallback,
		icon,
		tag,
		title,
		text,
		options,
		init,
		beforeOpen,
		beforeClose
	];
}

class PNotifyDesktopComponent extends SvelteComponent {
	constructor(options) {
		super();

		init_1(this, options, instance, null, safe_not_equal, {
			_notice: 0,
			desktop: 1,
			fallback: 2,
			icon: 3,
			tag: 4,
			title: 5,
			text: 6,
			options: 7,
			init: 8,
			beforeOpen: 9,
			beforeClose: 10
		});
	}

	get init() {
		return this.$$.ctx[8];
	}

	get beforeOpen() {
		return this.$$.ctx[9];
	}

	get beforeClose() {
		return this.$$.ctx[10];
	}
}

export default PNotifyDesktopComponent;
export { key, defaults, requestPermission };
//# sourceMappingURL=PNotifyDesktopComponent.js.map